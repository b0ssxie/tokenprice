...
`build` runs two steps: data fetch then Vite build. `node src/fetch-prices.js` runs alone in CI before vite.

## Architecture

- Single-page app, no framework. Entry: `index.html` → `src/main.js` imports `style.css`, `models.json`, `plans.json`
- `src/fetch-prices.js` — ESM script, fetches OpenRouter API, writes `src/data/models.json`. Filtered: models with negative pricing (prompt/completion < 0) excluded. Price per million tokens computed from per-token API values.
